package nl.odalitadevelopments.menus.utils.metrics;
...
public final class Metrics {
    ...
    private void startSubmitting() {
        final Runnable submitTask = () -> {
            if (!parentPlugin.isEnabled()) { // Plugin was disabled
                scheduler.shutdown();
                return;
            }
            // Nevertheless we want our code to run in the Bukkit main thread, so we have to use the Bukkit scheduler
            // Don't be afraid! The connection to the bStats server is still async, only the stats collection is sync ;)
            Bukkit.getScheduler().runTask(parentPlugin, this::submitData);
        };
    ...
    private void submitData() {
                        ...
                        if (plugin instanceof JsonObject) {
                            ...
                            try {
                                ...
                                if (logFailedRequests) {
                                    this.parentPlugin.getLogger().log(Level.SEVERE, "Encountered unexpected exception", e);
                                }
                            }
                        }
        ...
        new Thread(() -> {
            try {
                // Send the data
                sendData(parentPlugin, data);
            } catch (Exception e) {
                // Something went wrong! :(
                if (logFailedRequests) {
                    parentPlugin.getLogger().log(Level.WARNING, "Could not submit plugin stats of " + parentPlugin.getName(), e);
                }
            }
        }).start();
    }
